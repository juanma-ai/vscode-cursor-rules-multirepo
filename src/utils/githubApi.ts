...
// Define the factory function and its dependencies
export interface GithubApiDependencies {
  logger?: Logger;
}

// Factory function that creates the API client with injected dependencies
export function createGithubApiClient(
  ...
  return {
      ...
      try {
        ...
        if (fileExists) {
          const choice = await vscode.window.showQuickPick(
            ...
            {
              placeHolder:
                "How would you like to handle existing .cursorrules?",
            }
          );
    ...
    // For testing - expose internal utilities
    _testing: {
      getRawGithubUrl,
      getRepoIdentifier,
      convertGithubUrlToApi,
      processApiResponse,
    },
  };
}

// Create and export a default instance with standard dependencies
const defaultClient = createGithubApiClient();

// For backward compatibility, export the functions directly
export const fetchCursorRulesList = defaultClient.fetchCursorRulesList;
export const fetchCursorRuleContent = defaultClient.fetchCursorRuleContent;
